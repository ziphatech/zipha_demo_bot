...
const { approveCallback, appealCallback } = require("./menuButtonsCallback");
...
exports.menuOptionsCallback = async (ctx) => {
  ...
  // console.log(option,"just checking...")
  ...
  if (navigationAction) {
    try {
      // Navigate to the specified navigation action
      ...
      console.error("Error during navigation action:", error);
  ...
  try {
    switch (action) {
      case "approve":
        ...
        // const allUsers = await screenshotStorage.getAllUsers();
        // const allScreenshots = await screenshotStorage.getAllScreenshots(userId)
...
function handleError(ctx, error) {
  if (error.response && error.response.status === 429) {
    ctx.reply(
      "Oops! Our servers are a bit busy right now. Please try again later."
    );
  } else if (error.response && error.response.status === 400) {
    ctx.reply("Oops! Something went wrong with the request. Please try again.");
